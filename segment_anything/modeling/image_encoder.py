...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.MLP_Adapter = Adapter(embed_dim, skip_connect=False)  # MLP-adapter, no skip connection
        self.Space_Adapter = Adapter(embed_dim)  # with skip connection
        self.scale = 0.5
        ...
        for i in range(depth):
            block = Block(
                MLP = self.MLP_Adapter,
                Space = self.Space_Adapter,
                dim=embed_dim,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
...
class PromptGenerator(nn.Module):
    def __init__(self, scale_factor, prompt_type, embed_dim, tuning_stage, depth, input_type,
        ...
        self.shared_mlp = nn.Linear(self.embed_dim // self.scale_factor, self.embed_dim)  # adapter
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        adapter_dim = dim
        #################################################################################################
        self.MLP_Adapter = MLP  # MLP-adapter, no skip connection
        self.Space_Adapter = Space  # with skip connection
        self.scale = 0.5
        ################################################################################################
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        #print(sum(p.numel() for p in self.Space_Adapter.parameters() if p.requires_grad))
        shortcut = x
        x = self.norm1(x)
        # Window partition
